...
export async function GET(request: NextRequest) {
  ...
  // In split frontend architecture, if DATABASE_URL is not set on this service
  // but the backend API is reachable, the deployment is operational.
  if (!process.env.DATABASE_URL && backend.configured && backend.reachable) {
...
<body>
  <div class="card">
    ...
    <h1>System Status</h1>
    <p>Frontend service is operational and connected to the backend API.</p>
    <div class="metrics">
      <div class="metric-row"><span class="metric-label">Mode</span><span class="metric-val">Split Architecture (API Proxied)</span></div>
      <div class="metric-row"><span class="metric-label">Backend API</span><span class="metric-val">Reachable (200 OK)</span></div>
