...
export interface Section02CanvasRef {
  setFrame: (index: number) => void;
}

interface Section02CanvasProps {
  totalFrames?: number;
  className?: string;
}

const Section02Canvas = forwardRef<Section02CanvasRef, Section02CanvasProps>(
  ({ totalFrames = 60, className }, ref) => {
    ...
    useEffect(() => {
      ...
      for (let i = 1; i <= totalFrames; i++) {
        ...
        img.src = `/section2/ezgif-frame-${frameNum}.jpg`;
...
Section02Canvas.displayName = "Section02Canvas";

export default Section02Canvas;
