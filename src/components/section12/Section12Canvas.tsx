...
export interface Section12CanvasRef {
  setFrame: (index: number) => void;
}

interface Section12CanvasProps {
  totalFrames?: number;
  className?: string;
}

const Section12Canvas = forwardRef<Section12CanvasRef, Section12CanvasProps>(
  ({ totalFrames = 60, className }, ref) => {
    ...
    useEffect(() => {
      ...
      for (let i = 1; i <= totalFrames; i++) {
        ...
        img.src = `/section12/ezgif-frame-${frameNum}.jpg`;
      ...
      <div className="relative w-full h-full">
        ...
        {loadedCount < totalFrames && (
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full bg-black/80 border border-white/10 backdrop-blur-md text-[10px] font-mono text-neutral-400 z-30 pointer-events-none transition-opacity duration-500">
            PRELOADING RESPONSIVE ENGINE: {Math.round((loadedCount / totalFrames) * 100)}%
          </div>
        )}
      </div>
    );
...
Section12Canvas.displayName = "Section12Canvas";

export default Section12Canvas;
