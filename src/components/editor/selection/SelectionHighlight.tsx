...
export function SelectionHighlight({
  ...
  // Container specific popover states
  const [showContainerBgPopover, setShowContainerBgPopover] = useState(false);
  const [showContainerStylePopover, setShowContainerStylePopover] = useState(false);
  ...
  useEffect(() => {
    const handleOutside = (e: Event) => {
      ...
      setShowContainerBgPopover(false);
      setShowContainerStylePopover(false);
  ...
  const containerBg =
    ...
    (activeElement
      ? activeElement.style.backgroundColor || window.getComputedStyle(activeElement).backgroundColor
      : "transparent");
  const containerRadius =
    meta.radius ||
    meta.borderRadius ||
    (activeElement
      ? activeElement.style.borderRadius || window.getComputedStyle(activeElement).borderRadius
      : "0px");
  const containerPadding =
    meta.padding ||
    (activeElement
      ? activeElement.style.padding || window.getComputedStyle(activeElement).padding
      : "0px");
  const containerBorderWidth =
    meta.borderWidth ||
    (activeElement
      ? activeElement.style.borderWidth || window.getComputedStyle(activeElement).borderWidth
      : "0px");
  ...
  const handleContainerRadiusChange = (rad: string) => {
    const el = resolveElement();
    if (el) {
      el.style.setProperty("border-radius", rad, "important");
      el.dispatchEvent(new Event("input", { bubbles: true }));
  ...
  const handleContainerPaddingChange = (pad: string) => {
    const el = resolveElement();
    if (el) {
  ...
  const handleContainerBorderWidthChange = (w: string) => {
    const el = resolveElement();
    if (el) {
      el.style.setProperty("border-width", w, "important");
      el.style.setProperty("border-style", "solid", "important");
  ...
  const handleContainerBorderColorChange = (c: string) => {
    const el = resolveElement();
    if (el) {
      el.style.setProperty("border-color", c, "important");
      el.dispatchEvent(new Event("input", { bubbles: true }));
          ...
          <>
            ...
            <div className="relative">
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setShowContainerBgPopover(!showContainerBgPopover);
                  setShowContainerStylePopover(false);
                }}
                title="Container Background Color"
                className={`h-8 flex items-center gap-1.5 px-2.5 rounded-xl border text-[11.5px] font-medium transition cursor-pointer shrink-0 ${
                  showContainerBgPopover
                    ? "bg-slate-900 text-white border-slate-900 shadow-xs"
                    : "bg-slate-50 hover:bg-slate-100 text-slate-700 border-slate-200/80"
                }`}
              >
                ...
                />
                <span className="font-mono text-[11px] uppercase font-semibold">
                  {containerBg.startsWith("#") ? containerBg.toUpperCase() : "Bg"}
                </span>
                <ChevronDown className="w-3 h-3 opacity-50 shrink-0" />
              </button>
              ...
              {showContainerBgPopover && (
                ...
                >
                  ...
                  <div className="grid grid-cols-6 gap-1.5">
                    {PRESET_COLORS.map((hex) => (
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleContainerBgChange(hex);
                          setShowContainerBgPopover(false);
                        }}
                        className={`w-6 h-6 rounded-lg border transition hover:scale-110 shadow-xs cursor-pointer ${
                          containerBg.toLowerCase() === hex.toLowerCase()
                  ...
                  <div className="pt-2 border-t border-slate-100 flex items-center gap-2">
                    <input
                      type="color"
                      value={containerBg.startsWith("#") ? containerBg : "#ffffff"}
                    ...
                    <input
                      ...
                      value={containerBg}
                      onChange={(e) => handleContainerBgChange(e.target.value)}
                      className="flex-1 px-2 py-1 bg-slate-50 border border-slate-200 rounded-lg text-[11px] font-mono text-slate-800 uppercase focus:outline-none focus:border-blue-400"
                    />
                  </div>
                </div>
              )}
            ...
            <div className="relative">
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setShowContainerStylePopover(!showContainerStylePopover);
                  setShowContainerBgPopover(false);
                }}
                title="Container Style & Spacing"
                className={`h-8 flex items-center gap-1.5 px-2.5 rounded-xl border text-[11.5px] font-medium transition cursor-pointer shrink-0 whitespace-nowrap ${
                  showContainerStylePopover
                    ? "bg-slate-900 text-white border-slate-900 shadow-xs"
                    : "bg-slate-50 hover:bg-slate-100 text-slate-700 border-slate-200/80"
                }`}
              >
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="grid grid-cols-5 gap-1 flex-1 bg-slate-50 p-1 rounded-xl border border-slate-200/60">
                        {["0px", "1px", "2px", "3px", "4px"].map((w) => (
                          <button
                            key={w}
                            type="button"
                            onClick={() => handleContainerBorderWidthChange(w)}
                            className={`py-1 rounded-lg text-[10px] font-mono font-semibold text-center transition cursor-pointer ${
                              containerBorderWidth === w
                                ? "bg-white text-slate-900 shadow-xs font-bold border border-slate-200"
                                : "text-slate-600 hover:text-slate-900 hover:bg-white/60"
                            }`}
                          >
                      ...
                      <input
                        type="color"
                        value={containerBorderColor.startsWith("#") ? containerBorderColor : "#e2e8f0"}
                        onChange={(e) => handleContainerBorderColorChange(e.target.value)}
                        className="w-7 h-7 rounded-lg cursor-pointer border border-slate-200 bg-transparent p-0.5 shrink-0"
                        title="Border Color"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-5 gap-1 bg-slate-50 p-1 rounded-xl border border-slate-200/60">
                      {["none", "sm", "md", "lg", "xl"].map((sh) => (
                        <button
                          key={sh}
                          type="button"
                          onClick={() => handleContainerShadowChange(sh)}
                          className={`py-1 rounded-lg text-[10px] font-semibold uppercase text-center transition cursor-pointer ${
                            containerShadow === sh
                              ? "bg-white text-slate-900 shadow-xs font-bold border border-slate-200"
                              : "text-slate-600 hover:text-slate-900 hover:bg-white/60"
