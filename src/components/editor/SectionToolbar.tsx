...
import { DEVICE_META, ToolbarUtilities } from "./ToolbarUtilities";
...
export function SectionToolbar({
  ...
  const activeBackgroundControls = useMemo(() => {
    ...
    return {
      ...
      animation: backgroundGroup.controls.find((c) => c.id === "anim-preset"),
    };
  }, [backgroundGroup]);
  ...
  return (
    ...
    >
      ...
      <header className="flex shrink-0 items-center justify-between gap-2.5 border-b border-slate-200/80 px-3 py-1.5 bg-white">
        ...
        {/* Right: the utilities every toolbar carries, plus this one's own. */}
        <ToolbarUtilities
          device={device}
        ...
        {activeGroup?.id === "background" && activeBackgroundControls ? (
          <div className="py-0.5">
            <SingleRowBackgroundPanel
              ...
              animationValue={activeBackgroundControls.animation ? displayValue(activeBackgroundControls.animation) : ""}
              animationOptions={activeBackgroundControls.animation?.options ?? []}
              onCommitAnimation={(val) => {
                if (activeBackgroundControls.animation) commit(activeBackgroundControls.animation, val);
              }}
            />
          </div>
        ) : activeGroupId === "buttons" ? (
