...
export const InfiniteMovingCards = ({
  ...
  const getSpeed = () => {
    if (containerRef.current) {
      const duration =
        speed === "fast" ? "40s" : speed === "normal" ? "80s" : "120s";
    ...
    >
      ...
      >
        {items.map((item, idx) => (
          <li
            key={item.name + idx}
            className="relative w-[350px] max-w-full shrink-0 rounded-2xl border border-b-0 border-zinc-800 bg-[linear-gradient(180deg,#1a1a1e,#111113)] px-8 py-6 md:w-[450px]"
          >
            <blockquote>
              <div
                aria-hidden="true"
